...
import CameraReferencial from './extra-components/camera-reference/CameraReference'
...
const ThreejsBackround = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      console.log(windowWidth - previusWindoWidth)
    }, 1000)

    if (windowWidth === 1150
      || windowWidth - previusWindoWidth >= 2
      || windowWidth - previusWindoWidth < 1
    ) {
      location.reload()
      setTimeout(() => setDarkMode(!darkMode), 3000)
    }
  ...
  return (
    ...
    >
      <ambientLight color={`${ darkMode ? 'blue' : 'white' }`} intensity={`${ darkMode ? 0.5 : 2 }`} />
      <axesHelper args={[10]} />
      <CameraReferencial />
      ...
      <Suspense fallback={null}>
        <Sphere
          position={windowWidth > 1150 ? [7, 4.4, 6] : [6.77, 4.36, 6.05]}
          rotation-x={1}
          darkMode={darkMode}
        />
      </Suspense>
